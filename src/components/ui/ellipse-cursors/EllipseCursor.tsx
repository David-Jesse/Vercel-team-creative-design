...
// CONFIG
const NUM_BADGES = 26; // total number of badges
const radiusX = 420; // Wider horizontally
const radiusY = 200; // closer vertically

const verticalGapHeight = 100; // Target gap height left-right badges 90-100px
...
const badgeColors: BadgeColor[] = [
  ...
  { bg: "#ff9300", text: "#000000" }, // Orange
];

type Props = {
  shouldAnimate: boolean; // Only when the section is visible
};

const EllispseCursors = ({ shouldAnimate }: Props) => {
  return (
    <div className="ellipse-overlay">
      {Array.from({ length: NUM_BADGES }).map((_, i) => {
        const angle = (i / NUM_BADGES) * Math.PI * 2;
        ...
        // Slower, smoother floating transitions
...
export default EllispseCursors;