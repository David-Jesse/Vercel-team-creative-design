 ...
 * @param triggerRef - A ref to the DOM element to observe to observe (usually a 'sentinel' placed  near the component to pin).
...
export function usePinOnScroll(
  ...
  useEffect(() => {
    ...
    if (!triggerElement) return; // early exit
    ...
    // Cleanup observer on unmount or ref/options change
    return () => {
      observer.disconnect();
    };
  }, [triggerRef, options]);
